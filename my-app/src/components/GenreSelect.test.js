...
describe("GenreSelect Component", () => {
  ...
  test("renders all genres passed in props", () => {
    render(<GenreSelect genres={genres} selectedGenre="" />);
  ...
  test("highlights the selected genre passed in props", () => {
    ...
    expect(selectedButton).toHaveStyle("background-color: blue");
  });

  test("calls onSelect callback with correct genre on button click", () => {
    const onSelectMock = jest.fn();
    render(
      <GenreSelect genres={genres} selectedGenre="" onSelect={onSelectMock} />
    );
